...
class KDTree
{
  ...
  private:
     ...
     */
    void KD_helper(vector<Point<Dim>>& list, int d, int a, int b);

    int partition(vector<Point<Dim>>& list, 
            int left, int right, int pivotIndex, int d);

    void quickSelect(vector<Point<Dim>>& list,
            int d, int a, int b, int k);

    Point<Dim> NN_search(const Point<Dim>& list, 
