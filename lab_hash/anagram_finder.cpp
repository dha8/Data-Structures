...
bool AnagramFinder<Dict>::checkWord(const string& word, const string& test)
{
    ...
    Dict<char,int> hashTable(27); // 27 letters in alphabet    
    // mark counters for each letter
    for(unsigned i=0;i<word.size();++i) hashTable[word[i]]++;
    for(unsigned i=0;i<test.size();++i) hashTable[test[i]]--;
    
    bool ret = true;
    typename Dict<char,int>::iterator it;
    for(it=hashTable.begin();it!=hashTable.end();it++){
        if(it->second != 0) ret = false;
    }
    return ret;
}
