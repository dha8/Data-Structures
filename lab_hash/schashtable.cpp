...
void SCHashTable<K, V>::remove(K const& key)
{
    ...
    size_t idx = hash(key,size);
    // iterate through the chain, erase when found
    for(it=table[idx].begin();it!=table[idx].end();it++)
        if(it->first==key) table[idx].erase(it);
    --elems;
}
...
void SCHashTable<K, V>::resizeTable()
{
     ...
     */
    // find new size
    size_t newSize = findPrime(size);
    // create new table
    std::list<std::pair<K,V>>* newTable = 
        new std::list<std::pair<K,V>>[newSize];
    // copy over existing value
    ...
    // delete original table, reassign members
    delete[] table;
    table = newTable;
    size = newSize;
}
