...
class BinaryTree
{
  ...
  private:
    ...
    // private helper fxn for mirror()
    void mirrorize(Node* subRoot);

    // private helper fxn for isOrdered()
    void isOrdered_helper(const Node * subRoot, queue<T> & q) const;

    // private helper fxn for paths
    void pathHelper(vector<vector<T>> & paths, Node* n, int & idx) const;

    // private helper fxn for sumDistances()
    void sumHelper(const Node* n, int level, int & sum) const;
