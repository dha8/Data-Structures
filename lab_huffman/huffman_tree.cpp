                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if(mergeQueue.front()->freq.getFrequency() >= 
            singleQueue.front()->freq.getFrequency()){
        smallest = singleQueue.front();
        singleQueue.pop();
        return smallest;
    }
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while(singleQueue.size() + mergeQueue.size() > 1){
        TreeNode* one =removeSmallest(singleQueue, mergeQueue);        TreeNode* two =removeSmallest(singleQueue, mergeQueue);
        comb = new TreeNode(one->freq.getFrequency()
                +two->freq.getFrequency());
        comb->left = one;
        comb->right = two;
        mergeQueue.push(comb);
    }
    root_ = comb;
}
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
        ...
        // always start traversing from the root node
        if(curr->left==NULL && curr->right==NULL){
            ss << curr->freq.getCharacter();
            curr = root_;
            continue;
        }
        curr = (bfile.getNextBit()) ? curr->right : curr->left;
    }
    ss << curr->freq.getCharacter();
}
