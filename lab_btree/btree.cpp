...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
     ...
     */
    // couldn't find at leaf level, return default V.
    if(subroot->is_leaf) return V();

    // not at leaf, recurse to children
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    // case 1: is the key in the node?
    if(subroot->elements.size()!=first_larger_idx && 
            subroot->elements[first_larger_idx] == pair) return;
    // case 2: middle nodes, traverse til leaf
    BTreeNode* childptr = NULL;
    if(!subroot->is_leaf){
        childptr = subroot->children[first_larger_idx];
        insert(subroot->children[first_larger_idx], pair);
        if(childptr->elements.size() == order) 
            split_child(subroot, first_larger_idx);
        return;
    }
    // if leaf, actual insertion
    if(subroot->is_leaf)
        subroot->elements.insert(subroot->elements.begin() +
                first_larger_idx, pair);
}
